...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=2, in_chans=3, embed_dim=96, kernel_size=None):
        super().__init__()
...
class ShadowRemovalFusion(nn.Module):
    def __init__(self, dim, height=2, reduction=8):
        super(ShadowRemovalFusion, self).__init__()
...
class ShadowMaskFormer(nn.Module):
    def __init__(self, in_chans=3, out_chans=4, window_size=10,
        ...
        super(ShadowMaskFormer, self).__init__()
        ...
        self.fusion1 = ShadowRemovalFusion(embed_dims[3])
        ...
        self.fusion2 = ShadowRemovalFusion(embed_dims[4])
    ...
    def forward_features(self, x, x_mask, s_mask):
        # Two steps of Mask Augmented
        weighted_mask = self.w1 * s_mask + self.w2 * (1-s_mask)
        x = weighted_mask * x
        x = x * x_mask
        x = self.proj(x)  #target-feature
    ...
    def forward(self, x, x_mask):
        # Mask Augmented Patch Embedding

        H, W = x.shape[2:]
        x = self.check_image_size(x)
        s_mask = x_mask * 0.5 + 0.5
        # -1 / 1 mask
        x_mask = self.check_image_size(x_mask)
        # 0 / 1 mask
        s_mask = self.check_image_size(s_mask)
        #condmask = self.check_image_size(condmask)
        feat = self.forward_features(x, x_mask,s_mask)
...
def shadowmaskformer_s():
    return ShadowMaskFormer(
        embed_dims=[24, 48, 96, 48, 24],
        mlp_ratios=[2., 4., 4., 2., 2.],
        depths=[12, 12, 12, 8, 8],
        num_heads=[2, 4, 6, 1, 1],
        attn_ratio=[1/4, 1/2, 3/4, 0, 0],
        #attn_ratio=[1 / 2, 1 / 2, 1, 1/4, 0],
        conv_type=['DWConv', 'DWConv', 'DWConv', 'DWConv', 'DWConv'])
        #conv_type=['Conv', 'Conv', 'Conv', 'Conv', 'Conv'])

def shadowmaskformer_b():
    return ShadowMaskFormer(
        embed_dims=[24, 48, 96, 48, 24],
        mlp_ratios=[2., 4., 4., 2., 2.],
        depths=[16, 16, 14, 8, 8],
